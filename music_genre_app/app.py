...
def _load_genre_classification_models():
    ...
    이 함수가 호출될 때 관련 라이브러리(tensorflow, librosa, pandas, numpy, pickle, json)를 임포트합니다.
    """
    ...
    try:
        ...
        # model.summary() # 모델 구조 출력은 개발 시에만 필요하므로 제거
...
# pydub 및 ffmpeg 가용성 확인 로깅 (MP3 지원을 위해 다시 추가)
try:
    from pydub import AudioSegment
    # AudioSegment.converter는 ffmpeg 경로를 나타냅니다.
...
def extract_features_from_audio(file_path):
    ...
    try:
        ...
        y, sr = librosa.load(file_path, sr=44100) # 원본 샘플링 레이트 유지
        end_time = time.time()
...
def predict_genre_endpoint():
    ...
    try:
        from pydub import AudioSegment # 이 위치에서 임포트하여 필요할 때만 로드되도록 유지
        ...
        if filename.endswith('.mp3'):
            ...
            try:
                logging.info("MP3 to WAV 변환 시작...")
                audio = AudioSegment.from_mp3(temp_mp3_path)
                audio.export(temp_wav_path, format='wav')
                logging.info(f"MP3 파일 '{filename}'을 WAV로 변환 후 임시 저장: {temp_wav_path}")
                logging.info(f"MP3 to WAV 변환 완료. ({time.time() - file_processing_start_time:.2f}s)")
            except Exception as e:
